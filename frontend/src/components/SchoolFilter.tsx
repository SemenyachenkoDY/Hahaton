...
export const SchoolFilter: React.FC<SchoolFilterProps> = ({
  ...
  return (
    <div
      className="school-filter-container"
      ref={containerRef}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", flex: 1 }}>
          {selectedOgrns.length === 0 ? (
            <span style={{ color: "#666" }}>
              Выберите школы (ОГРН или Название)
            </span>
          ) : (
              ...
              filteredSchools.map((school) => (
                <div
                  ...
                  style={{
                    ...
                    justifyContent: "space-between",
                    alignItems: "center",
                    background: selectedOgrns.includes(school.ogrn)
                      ? "rgba(255, 102, 0, 0.05)"
                      : "transparent",
                    transition: "all 0.2s",
                  }}
                >
                  ...
                  {selectedOgrns.includes(school.ogrn) && (
                    <Check size={16} color="#cc3300" />
                  )}
                </div>
              ))
            ) : (
